...
describe('+ FilesController', () => {
  const baseDir = `${process.env.FOLDER_PATH || ''}`.trim().length > 0
    ...
    : joinPath(tmpdir(), process.env.DEFAULT_ROOT_FOLDER)
  const mockUser = {
    ...
    Promise.all([dbClient.usersCollection(), dbClient.filesCollection()])
      .then(([usersCollection, filesCollection]) => {
        Promise.all([usersCollection.deleteMany({}), filesCollection.deleteMany({})])
          ...
          .catch((deleteErr) => callback(deleteErr)) // Replace 'done' with 'callback'
      }).catch((connectErr) => callback(connectErr)) // Replace 'done' with 'callback'
  }
    ...
    it('+ Fetches first page with no page query', function (done) {
      ...
      request.get('/files')
        ...
        .end((requestErr, res) => {
          ...
          assert.isTrue(res.body.some((file) => file.name === mockFiles[0].name))
          assert.isTrue(res.body.some((file) => file.name === mockFiles[1].name))
          done()
        })
    })
    ...
    it('+ Fetches first page with no page query and parentId', function (done) {
      ...
      request.get(`/files?parentId=${mockFiles[1].id}`)
        ...
        .end((requestErr, res) => {
          ...
          assert.isTrue(res.body.some((file) => file.name === mockFiles[2].name))
          done()
        })
    })
